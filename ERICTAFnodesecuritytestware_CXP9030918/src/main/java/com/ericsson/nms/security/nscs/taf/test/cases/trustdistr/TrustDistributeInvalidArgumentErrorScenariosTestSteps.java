 ...
 * @author enmadmin
 */
public class TrustDistributeInvalidArgumentErrorScenariosTestSteps extends NodeSecurityTestCaseHelper {
    
    private static Logger logger = Logger.getLogger(TrustDistributeInvalidArgumentErrorScenariosTestSteps.class);

    @TestStep(id = "trustDistributeInvalidArgumentErrorTest")
    public void trustDistributeInvalidArgumentErrorTest(
            @Input("nodeList") String nodeList,
            @Input("ipAddressList") final String ipAddressList
            ) {

        System.out.println(" \n\n ############# Starting test trustDistributeInvalidArgumentErrorTest ############# \n\n");
...
//        final String[] ipArray = ipAddressList.split(",");
		...
		executeScriptEngineCommandWithFileAndVerifyResponseContainsExactErrorAndSuggestedSolution(TrustDistributeOperatorImpl.getTrustDistributeCommandNodeFile(NscsServiceGetter.CERT_TYPE_INVALID, NscsServiceGetter.TRUST_NODE_FILE),
        		NscsServiceGetter.TRUST_NODE_FILE,
        		createByteArrayWithNodeFileForSecadmCommand(nodeName),
                expectedError,
                expectedSuggestedSolution);
			
		executeScriptEngineCommandAndVerifyResponseContainsExactErrorAndSolution(TrustDistributeOperatorImpl.getTrustDistributeCommandNodeList(NscsServiceGetter.CERT_TYPE_INVALID, nodeName),
                expectedError,
                expectedSuggestedSolution);
