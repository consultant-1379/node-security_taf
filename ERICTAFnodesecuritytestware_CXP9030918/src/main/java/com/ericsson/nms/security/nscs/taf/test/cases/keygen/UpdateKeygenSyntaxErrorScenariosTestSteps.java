 ...
 * @author enmadmin
 */
public class UpdateKeygenSyntaxErrorScenariosTestSteps extends NodeSecurityTestCaseHelper {
  
    private static Logger logger = Logger.getLogger(UpdateKeygenSyntaxErrorScenariosTestSteps.class);

    @TestStep(id = "updateKeygenSyntaxError")
    public void updateKeygenSyntaxError(
            @Input("command") final String command,
            @Input("purpose") final String purpose){

        System.out.println(" \n\n ############# Starting test KeygenUpdateSyntaxErrors  ############# ");
        System.out.println("   #############     testing " + purpose + "    ############# \n\n");
        logger.info("test KeygenUpdateSyntaxErrors:" + " #############     testing " + purpose + "    #############");
...
//        Assert.assertEquals(httpResponseBody.contains(NscsServiceGetter.SUGGESTED_SOLUTION_PLEASE_CHECK_ONLINE_HELP_FOR_CORRECT_SYNTAX), true, "Test Case Failure : Expected Suggested not received" + NscsServiceGetter.SUGGESTED_SOLUTION_PLEASE_CHECK_ONLINE_HELP_FOR_CORRECT_SYNTAX);
        ...
        executeScriptEngineCommandAndVerifyResponseContainsErrorAndSolution(command,NscsServiceGetter.ERROR_10001_COMMAND_SYNTAX_ERROR,
